...
def get_reset_password_token() -> str:
    """
    Handles the POST request for the /reset_password endpoint.

    Form data:
        - email (str): The user's email address.

    Returns:
        A JSON payload with the reset token or aborts with a 403 status.
    """
    email = request.form.get('email')
    try:
...
def update_password() -> str:
    """
    Handles the PUT request for the /reset_password endpoint to update password.
    ...
    Returns:
        A JSON payload indicating success or aborts with a 403 status.
    """
    email = request.form.get('email')
